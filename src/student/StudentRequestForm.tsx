...
import type { ConsultationRequest, ConcernType, ConsultationMode } from "../types";
import { CONCERN_TYPES, SUBJECTS, FACULTY } from "../data";

interface Props {
  onSubmit: (req: Omit<ConsultationRequest, "id" | "status" | "priorityScore" | "waitDays" | "displacementCount" | "createdAt">) => void;
}

export default function StudentRequestForm({ onSubmit }: Props) {
    ...
    return (
      <div>
        <div className="flex items-start justify-between mb-6">
          <h1 className="font-display font-bold text-2xl text-slate-900">Request Consultation</h1>
        </div>
        <div className="max-w-lg mx-auto">
          <div className="card p-10 text-center">
            ...
            <h2 className="font-display font-bold text-slate-900 text-xl mb-2">Request Submitted!</h2>
            ...
            <div className="rounded-xl p-4 text-left mb-6" style={{ background: "#f8faff", border: "1px solid #e2e8f0" }}>
              <div className="grid grid-cols-2 gap-2 text-xs">
                ...
                <div><span className="text-slate-400">Preferred Date:</span> <span className="text-slate-700 font-medium">{form.preferredDate}</span></div>
                <div><span className="text-slate-400">Preferred Time:</span> <span className="text-slate-700 font-medium">{form.preferredTime}</span></div>
      ...
      <div className="max-w-2xl">
        ...
        {step === 1 && (
          <div className="card p-6 space-y-5">
            <h2 className="font-display font-semibold text-slate-800 text-lg">Consultation Details</h2>
        ...
        {step === 2 && (
          <div className="card p-6 space-y-5">
            <h2 className="font-display font-semibold text-slate-800 text-lg">Schedule Preferences</h2>
            ...
            <div className="rounded-xl p-4" style={{ background: "#fff7ed", border: "1px solid #fed7aa" }}>
              <div className="flex items-start gap-2">
                ...
                <p className="text-sm text-orange-700">Submitting a consultation request does <strong>not</strong> automatically reserve the requested time slot. The faculty member must approve your request before it is confirmed.</p>
              </div>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div>
                <label className="text-sm font-medium text-slate-700 mb-1.5 block">Preferred Date <span className="text-red-400">*</span></label>
                <input type="date" min="2026-09-15" className="w-full rounded-xl px-4 py-2.5 text-sm bg-white" style={{ border: "1px solid #e2e8f0" }} value={form.preferredDate} onChange={e => update("preferredDate", e.target.value)} />
              </div>
              <div>
                <label className="text-sm font-medium text-slate-700 mb-1.5 block">Preferred Time <span className="text-red-400">*</span></label>
                <input type="time" className="w-full rounded-xl px-4 py-2.5 text-sm bg-white" style={{ border: "1px solid #e2e8f0" }} value={form.preferredTime} onChange={e => update("preferredTime", e.target.value)} />
              </div>
            </div>

            <div>
              <label className="text-sm font-medium text-slate-700 mb-1.5 block">Estimated Duration</label>
        ...
        {step === 3 && (
          <div className="card p-6 space-y-5">
            <h2 className="font-display font-semibold text-slate-800 text-lg">Review & Submit</h2>
