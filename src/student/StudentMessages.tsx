...
export default function StudentMessages({ conversations, selectedId, onSelect, onSend }: Props) {
              ...
              conversations.map(conv => (
                <button key={conv.id} onClick={() => onSelect(conv.id)} className={`w-full text-left p-4 transition-all ${selectedId === conv.id ? "bg-blue-50" : "hover:bg-slate-50"}`} style={{ borderBottom: "1px solid #f8fafc" }}>
                  <div className="flex items-start gap-3">
                    <div className="w-9 h-9 rounded-full flex items-center justify-center text-white text-sm font-bold flex-shrink-0" style={{ background: "linear-gradient(135deg, #1d4ed8, #059669)" }}>MS</div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between">
                        ...
                        {conv.unread > 0 && <span className="badge ml-1">{conv.unread}</span>}
                      </div>
                      <div className="text-xs text-slate-400 truncate mt-0.5">{conv.subject}</div>
                      <div className="text-xs text-slate-500 truncate">{conv.lastMessage}</div>
                      <div className="text-xs text-slate-300">{formatTimestamp(conv.lastTimestamp)}</div>
                    </div>
                  </div>
                </button>
              ))
          ...
          <div className="flex flex-col flex-1 min-w-0">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3">
              {selected.messages.map((msg: ChatMessage) => {
                const isMe = msg.senderId === "s1";
                return (
                  <div key={msg.id} className={`flex ${isMe ? "justify-end" : "justify-start"}`}>
                    {!isMe && (
                      <div className="w-7 h-7 rounded-full flex items-center justify-center text-white text-xs font-bold mr-2 flex-shrink-0 self-end" style={{ background: "linear-gradient(135deg, #1d4ed8, #059669)" }}>MS</div>
                    )}
                    <div className="max-w-xs lg:max-w-md">
                      <div className={`rounded-2xl px-4 py-2.5 text-sm ${isMe ? "text-white rounded-br-sm" : "text-slate-800 rounded-bl-sm"}`} style={{ background: isMe ? "linear-gradient(135deg, #059669, #10b981)" : "white", border: isMe ? "none" : "1px solid #e2e8f0" }}>
                        {msg.content}
                      </div>
            ...
            <div className="p-4" style={{ borderTop: "1px solid #f1f5f9" }}>
              <div className="flex gap-2">
                ...
                />
                <button onClick={handleSend} disabled={!draft.trim()} className="w-10 h-10 flex items-center justify-center rounded-xl text-white transition-all hover:opacity-90 disabled:opacity-40" style={{ background: "linear-gradient(135deg, #059669, #10b981)" }}>
